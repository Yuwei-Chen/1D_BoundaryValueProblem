...
ua = truevd(ax); ub = truevd(bx); % Dirichlet BC
errg = zeros(1, ntimes);
...
for nn = 1:ntimes
    ...
    hx_u = (bx-ax)/n;
    gridx_u = ax + hx_u*[0:n];
    gridx = gridx_u.^4; gridxu = gridx;

    %solving PDE
    [rhs, coefs] = rhscfd(gridx, ua, ub);
    [ A, T2, T1, T0 ] = cfdmat(gridx, coefs);
    uvct = A\rhs;
    
    %Inf-norm abs error (approximation and true value)
    errg = errorfd(ngrid, gridx, n, [ua; uvct; ub], nn, errg);
end
